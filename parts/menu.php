<div class="modal modal--level-1" id="modal-menu">

    <div class="modal__background modal__background--black" data-modal-close></div>

    <div class="modal__content" style="opacity: 0">
        <div class="modal__close" data-modal-close>
            <svg viewBox="0 0 100 100" class="modal__close-icon-hover">
                <circle cx="50" cy="50" r="48" stroke="white" stroke-width=".5" fill="none"/>
            </svg>
            <svg class="modal__close-icon" viewBox="0 0 20.22 20.22">
                <polygon
                        points="18.8 0 10.11 8.7 1.41 0 0 1.41 8.7 10.11 0 18.8 1.41 20.22 10.11 11.52 18.8 20.22 20.22 18.8 11.52 10.11 20.22 1.41 18.8 0"/>
            </svg>
        </div>

        <div class="menu">

            <div class="light"></div>

            <div class="menu__block layout">
                <div class="menu__main">
                    <div class="menu__main-item menu__main-item--1" data-modal-close data-menu-redirect="portfolio">
                        <div class="char-hover">
                            <div class="char-hover__item">Portfolio</div>
                        </div>
                    </div>
                    <div class="menu__main-item menu__main-item--2" data-modal-close data-menu-redirect="process">
                        <div class="char-hover">
                            <div class="char-hover__item">Process</div>
                        </div>
                    </div>
                    <div class="menu__main-item menu__main-item--3" data-modal-close data-menu-redirect="services">
                        <div class="char-hover">
                            <div class="char-hover__item">Services</div>
                        </div>
                    </div>
                </div>

                <div class="menu__bottom">
                    <div class="menu__bottom-links">
                        <div class="menu__bottom-links-item" data-modal-close data-menu-redirect="about-as">
                            <div class="char-hover">
                                <div class="char-hover__item">About us</div>
                            </div>
                        </div>
                        <div class="menu__bottom-links-item" data-modal-close data-menu-redirect="clients">
                            <div class="char-hover">
                                <div class="char-hover__item">Clients</div>
                            </div>
                        </div>
                        <div class="menu__bottom-links-item" data-modal-close data-menu-redirect="contact-as">
                            <div class="char-hover">
                                <div class="char-hover__item">Contact us</div>
                            </div>
                        </div>
                    </div>

                    <div class="menu__bottom-contacts">
                        <div class="menu__bottom-contacts-mail">hello@svitlo-production.com</div>
                        <div class="menu__bottom-contacts-form-button" data-modal-open="modal-form">
                            <div class="char-hover">
                                <div class="char-hover__item">Let's discuss.</div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
</div>
